...
void SemanticAnalyzer::visit(UnaryExpr* node) {
    ...
    if (resultType == "void") {
        error("Invalid unary operation on type " + operandType,
              node->getLocation());
    }
        ...
        if (node->getType()) {
            ...
            if (!TypeChecker::areTypesCompatible(initType, type)) {
                error("Type mismatch in variable declaration. Expected " + 
                      type + ", got " + initType, node->getLocation());
            }
        } else {
            type = initType;
        }
...
void SemanticAnalyzer::visit(ReturnStmt* node) {
    if (node->getValue()) {
        ...
        if (!TypeChecker::areTypesCompatible(returnType, currentReturnType_)) {
            error("Return type mismatch. Expected " + currentReturnType_ + 
                  ", got " + returnType, node->getLocation());
        }
    } else {
...
void SemanticAnalyzer::visit(IfStmt* node) {
    ...
    if (!TypeChecker::isBoolType(condType)) {
        warning("Condition should be of type bool, got " + condType,
                node->getLocation());
    }
...
void SemanticAnalyzer::visit(LoopwhileStmt* node) {
    ...
    String condType = getExprType(node->getCond());
    if (!TypeChecker::isBoolType(condType)) {
        warning("Loop condition should be of type bool, got " + condType,
                node->getLocation());
    }
...
void SemanticAnalyzer::visit(FuncDecl* node) {
    ...
    if (!symbols_.declare(funcSymbol)) {
        error("Function " + node->getName() + " is already declared",
              node->getLocation());
    }
    ...
    for (const auto& param : node->getParams()) {
        ...
        if (!symbols_.declare(paramSymbol)) {
            error("Parameter " + param.name + " is already declared",
                  param.location);
        }
    }
